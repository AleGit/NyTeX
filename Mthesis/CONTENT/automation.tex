...
%	\end{gather*}
	which makes the next point even worser.
	...
	i.e.~in the worst case we have $c\times d$ pairs to check, see example \ref{ex:resolution:9a4}.
	This workload can be reduced with ordered resolution, which we will present in Section \vref{sec:ordered:resolution}.
	...
	are applicable. This can be addressed with equality axioms or equality inference rules as we will see in Section \vref{sec:proving:with:equality}.
\end{enumerate}
