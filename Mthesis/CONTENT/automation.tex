...
After that we look at more modern calculi for refutations based first order theorem proving 
...
\begin{example} For the formula $G(x)=\msucc(x)\mNE x$ we state the induction axioms in \CNF over the theory of natural numbers.
	\begin{gather*}
...
\begin{example}[Ordered resolution with equality axioms] 
	...
	$S = \{ \msucc(x)\mNE 0,\ \msucc(x)\mNE\msucc(y)\lor x\mEQ y \}$ and mark the maximal literals.
	\begin{align*}
	\colN x \mEQ x
	\tag*{$\mEQ$-reflexivity}
	\\
	...
	We only get a tautology from congruence and injectivity of $\msucc$.
	\begin{align*}
.\quad&
...
\begin{example}\label{ex:nat:instgen}
	When we look at the grounding for \InstGen we notice that the selection process is unfortunate,
	because the selected literals of any clause but congruence clash with $\msucc(x)\mNE 0$.
	\begin{align*}
...
\subsection{Paramodulation and Superposition}\label{sec:superposition}

\input{appendix/superposition}

\subsection{Inst-Gen-Eq}\label{sec:inst:gen:eq}
