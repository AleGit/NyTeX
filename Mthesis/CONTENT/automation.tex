...
Let us demonstrate a simple disadvantage of systematically generating instances of growing sizes.
...
\begin{enumerate}
	...
	The procedure will not terminate for any satisfiable sets
...
\begin{example} The selected literals of clauses may change between iterations.
%	1. p(a) | q(a)
%	2. p(a) | ~q(X)
%	3. ~p(X)
\begin{align*}
...
\section{Theorem proving with Equality}\label{sec:theorem:proving:with:equality}
...
\begin{theorem}\cite{Harrison:2009:HPL:1540610}
	...
	symmetry \axiom{S}, and
	transitivity \axiom{T} for the equality predicate.
	Further the congruence \axiom{C} must be satisfied 
...
\begin{definition}\label{def:minimal:conflicting:literals}
	A set of \coloremph{conflicting literals} is not satisfiable.
...
\begin{procedure}[Inst-Gen-Eq-Loop]\label{proc:inst:gen:eq:loop} \cite{KS2010ijcar}
...
Steps 4 and 5 are significant more work than in Procedure~\vref{proc:inst:gen:loop} with \InstGen.
...
\}
\end{align*}
Hence a satisfiable superset of \( S \) will never saturate.
