...
\begin{definition}[Consequence and validity]
...
% \begin{lemma}
% 	A simplification order $\succ_{\mathsf{gr}}$ on ground terms always exists.
% \end{lemma}

\subsection{Literal and Clause Orderings}\label{sec:literal:clause:orderings}

\IMPROVE{ DEFINITIONS of ORDERINGS, merge definitions }

% \begin{definiton}
% 	Multiset. $\{x,x \}$ is mutliset $M$ with $M(x)=2$
% \end{definiton}
