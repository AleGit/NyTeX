...
\begin{remark}
	...
	But if we want to check satisfiability of formulae \( F \) and we can transform \( F \) to a “simpler” formula \( G \) such that \( F \equisat G \) then it is sufficient to check \( G \) for satisfiability to show (un)satisfiability of \( F\).
\end{remark}
