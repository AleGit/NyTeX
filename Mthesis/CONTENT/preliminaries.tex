...
\begin{definition}
	A {\myem model} $\mcM$ for a set of clauses $S$ (for a sentence $F$)
	...
	A set of clauses (a sentence) is {\myem satisfiable} if there exists at least one model for it.
...
\begin{definition}
	...
	We write $\Gamma\entails G$ say that $\Gamma$ entails G.
\end{definition}
...
\begin{definition}[Equisatisfiability] Two sentences $F \equisat G$ are equisatisifiable if and only if $F$ is satisfiable if and only if $G$ is satisfiable.
\end{definition}

$\mP(\mf(x)) \not\equiv \mQ(\mg(x))$ but $\mP(\mf(x)) \equisat \mQ(\mg(x))$
...
\begin{theorem}[Gödels Vollständigkeitssatz]
...
\begin{definition}\label{def:rewriterule}
	...
	%	\]
	A rewrite rule $\ell'\rwStep r'$ is a {\myem variant} of $\ell\rwStep r$ if there is a variable renaming $\varrho$
	such that
