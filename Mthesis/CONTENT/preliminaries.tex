...
\subsection{Equisatisfiability}\label{sec:equisatisfiability}
...
% \begin{remark}
	...
	e.g.\ \(\mP(\ma) \equisat \mb\mEQ\mc \), which gives no insights over \( \mP(\ma) \) or \( \mb\mEQ\mc \). % chktex 21

	Otherwise when we can efficiently transform 
...
\begin{definition}\label{def:skolemization}
	...
	We define a \coloremph{skolemization} step \( \mcQ\,G \Rightarrow_{sk} \mcQ\,G' \)
...
% 
	...
	\( \mcQ''\,G'' \) is in \coloremph{skolem normal form} if \( \mcQ'' \)
