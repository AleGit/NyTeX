...
\begin{definition}
	...
	A (first order) sentence \( G \) is a \coloremph{semantic consequence} of a set of sentences
...
\begin{definition}
	Two sentences \( F \equisat S \) are \coloremph{equisatisfiable}
	if \( F \) is satisfiable whenever \( S \) is satisfiable
	and the other way round.
	An \coloremph{equisatisfiable transformation} morphs an arbitrary sentence
...
\begin{example}Equisatisfiability is easily extended to
	\[
	\{ \, \mP(x, \mf(x)), \, \mQ(y,\ma)\, \}
