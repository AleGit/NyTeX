...
%
In practice such a approach may fail for an unsatisfiable set of clauses
simply because an important clause pairing has been overlooked
...
we relied on a fair saturation process to prove completeness of \InstGenEQ{}.

In Section~\ref{sec:given:clause:algorithm}
we will combine the given clause algorithm
and a basic strategy to a a simple and fair saturation process
that eventually determines all possible derivate clauses
with respect to ordered resolution.
The same approach will fail easily for \InstGen{}.
...
\begin{procedure}[Given Clause Algorithm]
	...
	and \( U \) is the initially empty set of \coloremph{active} clauses.
	\begin{enumerate}
...
The crucial part is the selection of the given clause.
We have to ensure that any clause is selected eventually.
...
\begin{definition}[Oldest clause first strategy]
\end{definition}
...
\subsection{Fair saturation strategies}\label{{sec:fair:saturation:strategies:basics}}

Let us first demonstrate an insufficient saturation strategy with ordered resolution and the given claus algorithm.

\begin{example}[Newest clause first]
