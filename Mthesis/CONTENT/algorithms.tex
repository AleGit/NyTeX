...
and check each clause (selected literal) for the desired qualities.
	...
	for faster retrieval of matching clauses, literals, and terms. First we introduce the used notions for term indexing. After that we describe
	the use cases, data structures and algorithms for the index
	of clauses, literals, and terms and algorithms
	for a the fast retrieval of candidates for

	\begin{itemize}
		\item literals clashing with the selected literal of the given clause
		(\InstGen)
		\item unifiable subterms of literals with one side of the selected equation (\InstGenEQ{})
		\item variants of a subset of the given clause (Redundancy)
	\end{itemize}
...
%
		...
		\{ \ANGLES{\epsilon,x} \}
		& \text{if }t=x\in\SIGV \\
		\{ \ANGLES{\epsilon, \mf} \} \cup \{ \ANGLES{ip,s}\mid \ANGLES{p,s} \in\PosStr(t_i)\}
...
%
	\begin{example}
		...
		e.g.\ for term \( \mh(\mf(\ma, y )) \) we have \( \PosStr(
			{\mh(\mf(\ma, y ))}
			) = \{
			\ANGLES{\epsilon,\mh},
		\ANGLES{1,\mf},
...
\begin{notation}
	...
	\begin{gather*}
		...
		\simeq
		\ANGLES{p,\mh} \ANGLES{1,\mf} \ANGLES{2,y}
		\simeq
		p.\mh.1.\mf.2.y
		% \\
		% \ANGLES{\epsilon,\mh} \ANGLES{p1,\mf} \ANGLES{p12,y}
		% \simeq
		% \ANGLES{\epsilon,\mh} \ANGLES{1,\mf} \ANGLES{2,y}
		% \simeq
		% \mh.1.\mf.2.y
	\end{gather*}
\end{notation}
...
\begin{definition}
	We define the set of variable agnostic position strings \(\vaPosStr(t)\)
		...
		& \text{if }t=x\in\SIGV
		\\
		\{ \ANGLES{\epsilon, \mf} \} \cup \{ \ANGLES{ip,s}\mid \ANGLES{p,s}
		\in\vaPosStr(t_i)\}
		&\text{if }t=\mf(t_1,\ldots,t_n)
		\end{cases}
		\end{gather*}
...
\begin{example}
	Term paths and traversals work properly with
	variable agnostic positions strings.
	\begin{align*}
		\mg(\mf(x,y),z)
		&\Rightarrow
		\mg.1.\mf.1.*, \mg.1.\mf.2.*, \mg.2.*
		\tag*{term paths}
		\\
		&\Rightarrow \mg.\mf.*.*.*
