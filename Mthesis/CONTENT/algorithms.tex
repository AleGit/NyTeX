...
%
		We define the set of position strings \(\posS(t)\)
	...
	\begin{definition}
		...
		(see Definition~\vref{def:position}).
	\end{definition}
%
	\begin{example}
...
\begin{definition}
	...
	% variable occurrences \( \posV(t) \) as follows
	% \begin{gather*}
	% 	\posV(t) =
	% 	\begin{cases}
	% 	\{ \ANGLES{\epsilon,x} \}
	% 	& \text{if }t=x\in\SIGV \\
	% 	\{ \ANGLES{ip,s}\mid \ANGLES{p,s} \in\posV(t_i)\}
	% 	&\text{if }t=\mf(t_1,\ldots,t_n)
	% 	\end{cases}
	% 	\end{gather*}
A variable \(x\) occurs prior to variable \(y\) in a term \( t \)
if there exists a position \(p\) with \( \ANGLES{p,x} \in \posS(t) \)
such that position \(p\) is left of any position \(q\)
with \( \ANGLES{q,y} \in \posS(t) \).
%
A term \( \bar{t} \) is normalized
with sequence of variables \( {(x_i)}_{i=1}^n \)
if \( \var(\bar{t}) = \bigcup_{i=1}^n\{ x_i \} \)
and for any \( i < j \) the variable
\( x_i \) occurs prior to variable \( x_j \) in term \( \bar{t} \).
