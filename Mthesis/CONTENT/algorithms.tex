...
A crucial part in Procedure~\ref{proc:given:clause:algorithm} is the selection of the best unprocessed clause as the given clause.
...
\( S=\{ \, \mP(\ma)\lor\mQ(\ma), \mP(\ma)\lor\lnot\mQ(y), \lnot\mP(x) \, \} \).
	The processed and given clauses are already encoded and given to a \SAT{} solver.
...
Just moving processed clauses back to unprocessed clauses may introduce unnecessary derivations.
When a literal of a processed clause was de-selected
in iteration \( n \) and
is re-selected in a later iteration some of the pairings against
the selected literals of the processed clauses may have already been considered before iteration \( n \),
but we did not keep track of that.
