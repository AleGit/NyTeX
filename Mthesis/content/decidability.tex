...
\begin{definition}[\cite{MR1482227}]\label{def:prefix:class}
	...
	the absence or presence of subscript $(=)$ indicates whether the equality symbol is in the signature. 
...
\begin{example}\label{ex:monadic}
	...
	\end{gather*}
	The quantifier prefix $\Pi = all$ denotes an arbitrary sequence of quantifiers. 
\end{example}
...
\begin{example}\label{ex:ackermann}
	...
	\tag{Ackermann 1928}
	\end{gather*}
	The expression $all$ for $(p_1,p_2,\ldots)$ denotes $p_i=\omega$ for all $i\in\mathbb{N}$
\end{example}
