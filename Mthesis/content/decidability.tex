...
In this chapter we first recall that satisfiability in first order logic is undecidable in general 
and other famous theorems of first order logic in \ref{sec:decidable:fol:theorems}.
We describe all decidable fragments with purely syntactical descriptions 
in \ref{sec:decidable:fol:fragments}.
Then in \ref{sec:decidable:fol:theories} we look at decidable first order theories 
which may not be contained in one of the syntactically described decidable fragments of first order logic.
...
%\end{remark}
%
\begin{lemma}Equivalence and congruence of equality are provable by Natural Deduction (see Table \vref{tab:natural:deduction:rules}).\end{lemma}
\begin{proof}
