...
\begin{definition}\label{def:hk}
	An {\myem Herbrand universe} is the smallest set of terms that contains all $H_{i\geq 0}$ defined inductively as
	\begin{align*}
	H_0 &= \left\{ 
	\begin{array}{ll}
...
\begin{example}
	...
	However $\mcH$ is not an normal model because obviously $\mf(\ma)\neq\ma$ in its domain.
	%	
