...
%A (function) term is called {\myem linear} if it does not contain multiple occurrences of the same variable.
%
\end{definition} 
%% DEF %%
...
%A binary relation $R$ on terms is {\myem closed under contexts}
%if $u[s]|_p\,R\;u[t]|_p$ holds 
%for all positions $p\in\pos(u)$ and
%for all terms $s,t,u$ with $s\,R\;t$.
%It is {\myem closed under substitutions}
