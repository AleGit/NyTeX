...
\begin{remark}
	A proposition, i.e.~a constant or nullary predicate) $\mpp$ holds if $\mpp_\mcI = \{ () \} = A^0$.
\end{remark}

\begin{lemma}[Instantiation]\label{lem:instantiation}
	Let $S$ be a set of clauses and $\mcM$ a model for $S$.
	Then $\mcM\models S\sigma$ holds for any substitution $\sigma$.
\end{lemma}
%\begin{proof}
%	By induction of the finite cardinality $n = | \domain(\sigma)|$. Let $\mcM\models S$.
%	\begin{enumerate}
%		\item $n = 0$ then $S\sigma = S$
%		\item $n = 1$ Show that if $\mcM\models S$ then $\mcM\models S\{x\mapsto t\}$
%		(By induction on S)
%		\begin{enumerate}
%			\item If $P(c)$ holds, then $P(\mc)\sigma = P(\mc)$ holds
%			\item If $P(y)$ holds, then $P(y)\sigma = P(y)$ holds.
%			\item If $P(x)$ holds, then 
%			then $P(x)\sigma = P(t)$ holds.
%		\end{enumerate}
%		\item $S(\sigma\{x_0\mapsto t_0\} = (S\sigma)\{x_0\mapsto t_0\}$
%	\end{enumerate}
%\end{proof}
