...
\begin{definition}
	...
	definitions of (partial) functions $\mf^\mcI: A^n\rightarrow A$ for every function symbol $\mf\in\mcFf$, 
	...
	if and only if $(t_1,\ldots,t_n) \in \mP^\mcI$.
...
\begin{definition}
	A set of clauses $S$ is satisfiable if and only if there is a model $\mcM$ for $S$. 
	A set of clauses is valid if and only if all clauses hold in every interpretation.
\end{definition}
...
%	
	...
	with domain $\{ \mc \}$, function definition $\mf(\mc) = \mc$, 
	and $\mEQ$ is defined by equality in its domain.
