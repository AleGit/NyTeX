...
\begin{definition}\label{def:substitution}
	...
	respectively the application of a substitution $\sigma$ to a first order expression $\mct$:
	\DEFINE{
		\mct\sigma
	}{
			...
			\\
			\mcf(\mct_1\sigma,\ldots,\mct_n\sigma)	&\text{if }\mct=\mcf(\mct_1,\ldots,\mct_n), \mcf\in\mcFn
			\\
...
%			\text{ where }A\text{ is an atom}
			\text{ where } A\in\mcPT\disjointunion\mcET  \\
%			L_1\sigma\lor\ldots\lor L_n\sigma	&\text{if }\mkt=L_1\lor\ldots\lor L_n \\	
}
\
end{defintion}
\noindent Note: We use prefix and infix notation interchangable, e.g.
