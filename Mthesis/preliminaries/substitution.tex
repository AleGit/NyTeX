...
\begin{definition}\label{def:substitution}
	...
	\DEFINE{
		\mct\sigma
			...
			\\
			\mysymbol(\mct_1\sigma,\ldots,\mct_n\sigma)	&\text{if }\mct=\mysymbol(\mct_1,\ldots,\mct_n) 
			\text{ where  }
			 \mysymbol\text{ is not a quantifier}
}
...
\begin{definition}
	...
	and we abbreviate $F(x)\{x\mapsto t\}$ with $F(t)$, i.e.~all occurrences of the only free variable in $F$ is substituted with term $t$.
\end{definition}
