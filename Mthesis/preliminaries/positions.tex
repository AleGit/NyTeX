...
\begin{definition}
	...
	We define the set of term positions in term or atom $\foxt$,
	\DEFINE{ 
		\pos(\foxt) }
	{
		\{ \epsilon \} 		
	...
	the extraction of a subterm from a term or atom $\foxt$ at term position $p\in\pos(\foxt)$
%	
	\DEFINE{
		\foxt|_p
	}{
	...
	%
	and the insertion of a term $s$ at a term position $p\in\pos(\foxt)$
	\DEFINE{
		\foxt[s]_p}
	{
		s 		& \text{if }p=\epsilon \\
