...
\begin{definition}\label{def:natural:deduction}
	...
	With conditions
	$\fvar(F)\cup\fvar(G) = \emptyset$, 
	$\fvar(F') = \{ x \}$,
	$\var(s) \cup \var(t)\subseteq \bvar(F')$, and 
	$\var(u) = \emptyset$
	for occurring variables we ensure that we only can prove sentences, i.e.~formulae without free variables.
