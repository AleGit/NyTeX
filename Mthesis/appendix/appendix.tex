...
\begin{tikzpicture}
    ...
    \node (CUS) at (8,8) { substitutions };

    \node (IRR) at (-2,6) { irreflexive };
    \node (TRA) at (2,6) { transitive };
    \node (CL) at (6,6) { closed under };

    \node (PO) at (0,4) { proper/strict order };
    \node (RWR) at (4,4) { rewrite relation };
    
    \node (STP) at (-2,2) { subterm property };
    \node (RWO) at (2,2) { rewrite order };
    \node (WF) at (6,2) { well-founded };
