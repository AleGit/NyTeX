...
% 
    ...
    % \( \succC' \) --- an inherently well-founded order defined as extension of 
    \( \succG \)
    such that
...
\end{definition}
   ...
   \begin{definition}[Unit superposition and resolution on literal closures~\cite{CS2011PhD}, UPC]\label{def:unit:superposition:on:closures}
    \begin{gather*}
        \infer[(\sigma)]
    ...
    % \qquad
    %     \infer[(\sigma)]
    %     {(s[r]\mNE t)\sigma\cdot\rho}
    %     {(l\mEQ r)\cdot\theta_{\!_1} & (s[l']\mNE t)\cdot\theta_{\!_2}}
    % \qquad
    %     \infer[(\sigma)]
    %     {(s[r]\mEQ t)\sigma\cdot\rho}
    %     {(l\mEQ r)\cdot\theta_{\!_1} & (s[l']\mEQ t)\cdot\theta_{\!_2}}
    % \qquad
        \infer[(\sigma')]
        {\emptyclause}
        {(s'\mNE t')\cdot\theta'}
   ...
   \begin{remark}
    ...
    than without, which is a benefit because saturation is more likely. 
    However, it still has to yield a complete procedure
...
\noindent Note that we can easily demonstrate that UPC on its own 
is not complete on sets of unit closures,
i.e.~we might or might not be able to derive the empty clause 
from inconsistent set of literal closures.
First we look at a working example then at an “equivalent” counterexample.

\begin{example}
    With \( \ma\succG\mb \) 
    we can derive the empty clause from the set of unit closures
    \( \{
        (\mf(\ma)\mEQ\mb) \cdot\emptyset, 
        \ma\mEQ \mb \cdot\emptyset,
        \mf(\mb)\mNE\mb \cdot\emptyset
    \} \) in two steps.
    We start with a superposition step and finish with a resolution step, as follows.
\begin{gather*}
    \infer[\emptyset]{
        \emptyclause
    }{
...
\begin{example}
    With \( \ma\succG\mb \) 
    we cannot derive any unit closure from the set of unit closures
       \( \{
           \mf(x)\mEQ\mb \cdot\{ x\mapsto\ma \},
           \ma\mEQ \mb \cdot\emptyset,
           \mf(\mb)\mNE\mb \cdot\emptyset
       \} \). 
       Neither the resolution rule nor the superposition rule 
       are applicable to any of the unit closures.
\begin{align*}
    \infer[\emptyset]{
        {\colG \varepsilon}
    }{
    ...
    \infer[]{
        ...
        &\qquad \mgu(\ma, {\colLo\ell'}) \text{ does not exist}
    \\
    \infer[]{
        {\colG \varepsilon}
    }{
        ...
        &\qquad \mgu(\mf(x), {\colLo\ell'}) \text{ does not exist}
    % \tag*{not applicable}
\end{align*}
           \end{example}
...
is satisfiable if and only if its grounding is satisfiable.
   ...
   \begin{definition}[UP-Redundancy]
       ...
       \begin{itemize}
           \item The set of irreducible literal closures \( \IRRED_R(\mcL) =
           \{ \,
       ...
       \item The subset of smaller literal closures
       \(
...
\end{definition}
           ...
           \mcL_{i+1} = \left \{
                   \begin{array}{lclc}
                       \mcL_i \backslash L\cdot\sigma
                        &\text{if}
                        &R \cup \IRRED_R(\mcL_i)_{L\cdot\sigma\succL} \entails L\sigma
                       \\
                       \\
                       ...
                       \\
                       \mcL_i
                        &&\text{saturates if no other case is applicable}
                   \end{array}
               \right.
               \end{gather*}
   ...
   \begin{definition}[UP-Fairness]
            A UP-saturation process is \coloremph{UP-fair} if for every UP-inference
...
\begin{definition}[Inst-Redundancy]\label{def:inst:redundant}
   ...
   \begin{itemize}
       \item A ground closure \( C \) is \coloremph{Inst-redundant} in \( S \)
        if for some \( k \)
...
\begin{definition}[Inst-Saturation]
    Let \( \mcL_S^{sat} \) denote the result of the UP-saturation process of \( \mcL_S \).
    Then a set of clauses \( S \) is \coloremph{Inst-saturated} w.r.t.~a selection function \( \sel \),
                   ...
                   \item \( \epsilon_L = \left \{
                       \begin{array}{cll}
                           ...
                           \text{ or }
                            ...
                            &\text{i.e. a defined literal closure}
                           \\
                           \{ L'\sigma \}
                            &\text{otherwise}
                            &\text{i.e~a productive literal closure}
                       \end{array}
                   \right. \)
...
% 
            Then a minimal ground closure \(
                D = \min_{\succC} \{ \,
                C'\cdot\sigma \mid C'\in S, \,
               \mcI\not\models C'\sigma \,
               \} \),
                an instance of a clause in \( S \),
               \( \false \) in \( \mcI \), must exist. 
               Furthermore 
...
%    \IMPROVE{Section~\vref{sec:saturation:strategies}}
   ...
   \( S \) is satisfiable whenever \( S\bot \) is satisfiable.
