...
\begin{table}[hbt]	
	...
	x + 0 \mEQ x
	\\
	x + 1 \mNE y + 1 \lor x \mEQ y
	\\
	x+(y+1) \mEQ (x+y)+1
	\\
	\lnot G(0) \lor G(k) \lor G(x), \lnot G(0) \lor \lnot G(k+1) \lor G(x)
	\end{gather*}
	\caption{Clauses for Presburger arithmetic}
...
So far we has treated the equaltiy symbol like an predicate symbol.
...
We add add the equaltiy axioms to our set of clauses and see what happens.
