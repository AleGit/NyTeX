...
\begin{example}\label{ex:sat:nezsx}
	With satisfiable set $S_{\ref{ex:sat:nezsx}} = \{\, 
		\lnot\mE(\mzero,\ms(x)), \mE(y,y') \lor \lnot \mE(\ms(y),\ms(y')
		\,\}$
...
%	\\[1em]
	...
	[\{y\mapsto\msucc^i(\mzero), y'\mapsto\msucc^{i+1}(x)\}]
	{
		...
		{\colHi\mE(y,y')} \lor \lnot \mE(\ms(y),\ms(y')) 
		&{\colLo \lnot\mE(\ms^i(\mzero),\msucc^{i+1}(x))}
	}
	\tag{$i\geq 0$}
	\end{gather*}
\end{example}
...
\begin{gather*}
\infer[\{x\mapsto\msucc^k(y)\}]{
	{ \lnot\mE(\ms^k(y), \ms(\ms^k(y))))} \qquad
...
\begin{gather*}
\{\,
	\mE(\ms^k(\consbot), \ms(\ms^k(\consbot))), \lnot\mE(\ms^k(\consbot), \ms(\ms^k(\consbot)))) 
\,\}
\end{gather*}
