...
\begin{example}
	Let $S = \{\,\mE(\mzero,y)\,\}$ be the satisfiable set of example \ref{ex:satEzy}. 
	We can not derive any clause with InstGen.
\end{example}
...
{ \mE(\ms^k(y), \ms(\ms^k(y)))}
\,\}$ 
be the unsatisfiable set of example \ref{ex:unsatE2}. 
We immediately derive a new clause with InstGen.
\begin{gather*}
