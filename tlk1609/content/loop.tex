...
\node (ab) [myrect] at (-8em,3.5em) {encoding};
\node (gs) [myrect] at (-8em,8em) {SAT/SMT};
...
\node (parse) [myrect,dotted]  at (14em,-2em) {read tptp\\cnf file(s)};
...
\draw[myarrow, dashed] (gc) to [bend left=20] (ac);

\path (ac) edge [bend left, dotted] (us);
